...
#### 2.1.3.2.1. Régression Lasso OU L1
...
**3. Effets de la pénalité** 


- **Régularisation** : La pénalité $\lambda \sum_{j=1}^p |\beta_j|$ contraint certains coefficients $\beta_j$ à être exactement égaux à zéro lorsque $\lambda$ est suffisamment grand. Cela simplifie le modèle en éliminant les **variables non pertinentes**.

- **Sélection de variables** : Comme certains coefficients deviennent exactement nuls, LASSO effectue une sélection de variables. Seules les variables avec des coefficients non nuls sont retenues dans le modèle final.

- **Réduction de la variance** : En réduisant certains coefficients, LASSO peut aider à diminuer la variance du modèle, ce qui améliore la capacité du modèle à généraliser sur des données nouvelles.

**4. Choix du paramètre $\lambda$** 

- Le paramètre $\lambda$ est crucial et doit être choisi avec soin. Une méthode courante pour sélectionner $\lambda$ est la validation croisée. Par cette méthode, on évalue les performances du modèle pour différentes valeurs de $\lambda$ et on choisit celle qui minimise l'erreur de validation.

**5. Interprétation des résultats**

- Coefficients non nuls : Les variables associées à des coefficients non nuls sont considérées comme importantes pour la prédiction.

- Coefficients nuls : Les variables associées à des coefficients nuls sont éliminées du modèle, ce qui peut simplifier l'interprétation et réduire le sur-ajustement.



#### 2.1.3.2.2. Régression Ridge ou L2
...
**- Différence entre LASSO et RIDGE**

**1. Sélection de Variables**

- LASSO : Peut mettre certains coefficients à zéro, ce qui élimine complètement les variables correspondantes. C'est donc une méthode de sélection de variables.

- RIDGE : Réduit les coefficients mais ne les met pas à zéro. Toutes les variables restent dans le modèle.

**2. Régularisation et Complexité du Modèle**

- LASSO : Tends à produire des modèles plus simples en éliminant certaines variables, ce qui peut aider à l'interprétabilité.

- RIDGE : Réduit la complexité du modèle en réduisant l'ampleur des coefficients, mais garde toutes les variables.

**3. Stabilité et Colinéarité**

- LASSO : Peut être instable lorsque les variables sont fortement corrélées. Dans ce cas, il peut sélectionner arbitrairement une des variables corrélées.

- RIDGE : Fonctionne mieux avec des variables corrélées en répartissant la pénalisation sur les coefficients corrélés.

** 4. Impact de $\lambda$

- LASSO : À mesure que $\lambda$ augmente, plus de coefficients peuvent être réduits à zéro.

- RIDGE : À mesure que $\lambda$ augmente, les coefficients sont progressivement réduits, mais aucun n'atteint exactement zéro.

**- Utilisation en Pratique**

- LASSO : Utilisé lorsque l'on souhaite à la fois la régularisation et la sélection de variables, particulièrement utile dans les cas où certaines variables sont inutiles ou redondantes.

- RIDGE : Utilisé lorsque toutes les variables sont potentiellement utiles et que l'on souhaite simplement régulariser le modèle pour éviter le sur-ajustement.


LASSO et RIDGE sont des méthodes de régularisation complémentaires avec des caractéristiques distinctes. Le choix entre elles dépend de la nature des données et des objectifs spécifiques du modèle.
