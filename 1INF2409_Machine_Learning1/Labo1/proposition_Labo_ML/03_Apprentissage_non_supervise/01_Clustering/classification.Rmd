...
Comme d'habitude, avant tout, il faut toujours commencer par décrire la base de donnée afin de la comprendre le jeux de données et de mener des analyses descriptives préliminaires afin d'avoir une première du sujet mis à l'étude. Ici, la variable d'intérêt principale est l'état de survie de l'enfant; qui est une variable binaire dont l'objectif est de construction des modèles de classification pourvant mieux discriminer ses modalités. \
...
Dans le data train, il y'a un gros problème de déséquilibre de données. Construire un modèle sur de telles données aura tendance à privilégier le classement de la classe majoritaire. Dans ces conditions, il faut rééquilibrer les données. Pour cela il existe plusieurs technique d'équilibrage de donnée. Soit on suréchantillonne la classe minoritaire en augmentant de façon synthétique le nombre d'exemple de la classe minoritaire, soit en sous-échantillonnant la classe majoritaire. Dans le cadre de cette exemple, on va suréchantillonner la classe minoritaire par simple réplication aléatoire (il existe plusieurs autre technique ex: SMOTE)
...
En conclusion on constate que dans l'ensemble les algorithmes d'apprentissage automatique de type *boite noire* tant en mieux performés au vue des métriques ici considérées que les modèles de regression. En realité, le choix definitif du meilleur modèle doit tenir non seulement des métriques de performances mais aussi des objectifs de l'étude. 
